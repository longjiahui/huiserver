export const Code = {
    ...
    unexpectedFailed(code: number) {
        return Code.unexpectedError + code
    },
}
...
export function unexpectedFailed(reason: string, code?: number) {
    return fail(Code.unexpectedFailed(code ?? 0), reason)
}
